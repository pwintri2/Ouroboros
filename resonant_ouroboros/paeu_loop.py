...
class PAEUMetrics:
    ...
    hz_band_performance: dict[str, int] = field(default_factory=lambda: {
        "deep_read": 0,
        "curious_scan": 0,
        "creative_spike": 0
    })
...
class PAEULoop:
    ...
    async def learn_topic(self, topic: str, steps: int = 3) -> list[PAEUEvent]:
        ...
        for _ in range(max(1, steps)):
            ...
            if snapshot.quality_score > 0.5:
                ...
                self.metrics.hz_band_performance[behavior.mood] += 1
            else:
                record_id = None
                self.metrics.rejected_pages += 1
                safety_reason = f"rejected: {snapshot.quality_reason}"
    ...
    def store_seed_topic_without_browser(self, topic: str, reason: str = "dry_run") -> str:
        ...
        record = build_11d_record(
            ...
            path_or_proprioception="data/seeds/AGI Kennis.txt",
