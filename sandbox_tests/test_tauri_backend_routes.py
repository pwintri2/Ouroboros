...
class TestTauriBackendRoutes(unittest.TestCase):
    ...
    def test_project_context_routes_include_dash_and_underscore_aliases(self):
        ...
        local_summary = self.client.get("/context/summary?source=local")
        test_files = self.client.get("/context/test-files?limit=10&source=local")
        structure = self.client.get("/context/structure?source=local")
        ...
        self.assertEqual(local_summary.status_code, 200)
        self.assertEqual(test_files.status_code, 200)
        self.assertEqual(structure.status_code, 200)
        self.assertIn("structure", summary.json())
        self.assertEqual(local_summary.json()["resolved_source"], "local")
